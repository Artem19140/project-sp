class Figure {
  constructor({ color = "#ee8844", centre = new Point() }) {
    this.points = [];
    this.edges = [];
    this.polygons = [];
    this.animations = [];
  ...
  doAnimation(math3D) {
    this.animations.forEach((anim) => {
      const t1 = math3D.move(-anim.centre.x, -anim.centre.y, -anim.centre.z);

      const t2 = math3D[anim.method](anim.value);

      const t3 = math3D.move(anim.centre.x, anim.centre.y, anim.centre.z);
