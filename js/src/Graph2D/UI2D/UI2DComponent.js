class UI2DComponent extends Component {
  ...
  addEventListeners() {
    document
      .querySelector(".addFunction")
      .addEventListener("click", () => this.addFunctionHandler());
  ...
  addFunctionHandler() {
    const inputFunc = this.createInput(
      this.keyUpFunctionHandler,
      "f(x)",
      "inputFunc"
    );

    const inputWidth = this.createInput(
      this.keyUpWidthHandler,
      "Ширина",
      "inputWidth",
      "number"
    );

    const inputColor = this.createInput(
      this.keyUpColorHandler,
      "Цвет",
      "inputColor",
      "color"
    );

    const inputA = this.createInput(
      this.keyUpAHandler,
      "a",
      "inputA",
      "number"
