class UI3DComponent extends Component {
  ...
  addEventListeners() {
    document
      .querySelector(".lightPower")
      .addEventListener("change", () => this.lightHandler());
    document
      .querySelector(".menuGraph3DButton")
      .addEventListener("click", this.menuHandler);
    document
      .querySelector(".addFigure")
      .addEventListener("click", () => this.openFiguresListHandler());
    document.addEventListener("keypress", (event) =>
      this.callbacks.keyPress(event)
    );
    document
      .querySelectorAll(".figuresCheckBoxes > input")
      .forEach((checkBox) => {
        checkBox.addEventListener("change", (event) =>
          this.changeCheckBoxHandler(event)
        );
      });
    document
      .getElementById("animationCheckBox")
      .addEventListener("change", () => this.callbacks.animationHandler());
    document
      .getElementById("figuresList")
      .addEventListener("click", (event) => this.addFigureHandler(event));
  ...
  lightHandler() {
    this.callbacks.changeLightPower(
      document.querySelector(".lightPower").value - 0
    );
  }
  ...
  menuHandler() {
    ...
    if (this.menuOpen) {
      setTimeout(
        () => document.querySelector(".block3D").classList.toggle("hide"),
        100
      );
  ...
  addFigureHandler(event) {
    ...
    if (figure === "SunSystem") {
      ...
      document
        .querySelector(".figuresContainer")
        .appendChild(this.createSettings(figure));
    }
  }
  ...
  createSettings(figure) {
    const inputX = this.createInput(
      "number",
      "x",
      "x",
      "inputCenterPointFigure"
    );
    const inputY = this.createInput(
      "number",
      "y",
      "y",
      "inputCenterPointFigure"
    );
    const inputZ = this.createInput(
      "number",
      "z",
      "z",
      "inputCenterPointFigure"
    ...
    switch (figure) {
      ...
      case "Sphere":
        settingsBlock.appendChild(
          this.createInput("number", "radius", "Радиус")
        );
        settingsBlock.appendChild(
          this.createInput("number", "count", "Кольца")
        );
        break;

      case "Ellipsoid":
        settingsBlock.appendChild(
          this.createInput("number", "focusOx", "ФокусOx", "ellipsoidFocus")
        );
        settingsBlock.appendChild(
          this.createInput("number", "focusOy", "ФокусOy", "ellipsoidFocus")
        );
        settingsBlock.appendChild(
          this.createInput("number", "focusOz", "ФокусOz", "ellipsoidFocus")
        );
        settingsBlock.appendChild(
          this.createInput("number", "count", "Кольца")
        );
      ...
      case "Cylinder":
        ...
        settingsBlock.appendChild(
          this.createInput("number", "height", "Высота")
        );
        break;
      ...
      case "ParabolicCylinder":
        ...
        settingsBlock.appendChild(
          this.createInput("number", "width", "Ширина")
        );
        settingsBlock.appendChild(
          this.createInput("number", "height", "Высота")
        );
      ...
      case "HyperbolicCylinder":
        ...
        settingsBlock.appendChild(
          this.createInput("number", "focusOx", "ФокусOx")
        );
        break;
      ...
      case "DoubleCavityHyperboloid":
        ...
        settingsBlock.appendChild(
          this.createInput("number", "focusOy", "ФокусOy")
        );
        settingsBlock.appendChild(
          this.createInput("number", "focusOz", "ФокусOz")
        );
        break;
      ...
      case "Tor":
        settingsBlock.appendChild(
          this.createInput("number", "radius", "Внутренний радиус")
        );
        settingsBlock.appendChild(
          this.createInput("number", "radius2", "Радиус")
        );
        settingsBlock.appendChild(
          this.createInput("number", "count", "Кольца")
        );
